 ...
 */
export function setRivetContext(rivet) {
...
export function getRivetContext() {
    const ctx = getContext(RIVETKIT_CONTEXT_KEY);
    if (ctx === undefined) {
        throw new Error('Context "RivetKit" not found. Did you call setupRivetKit() or setRivetContext() in a parent layout?');
    }
    return ctx;
}
/**
 ...
 */
export function hasRivetContext() {
 ...
 * ```
 */
export function setupRivetKit(clientInput, opts) {
    const rivet = createRivetKit(clientInput, opts);
 ...
 * ```
 */
export function setupRivetKitWithClient(client, opts) {
    const rivet = createRivetKitWithClient(client, opts);
    return setRivetContext(rivet);
}
